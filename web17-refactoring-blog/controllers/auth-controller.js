...
function getLogin(req, res) {
	const sessionErrorData = validationSession.getSessionErrData(req, {
		email: '',
		password: '',
	});
  ...
  async function createUser(req, res) {
	...
	if (!validation.userCredentialAreValid(
		enteredEmail,
		enteredConfirmEmail,
		enteredPassword
	)) {
  ...
  async function connectLogin(req, res) {
	const userData = req.body;
	const enteredEmail = userData.email;
	const enteredPassword = userData.password;
  
	const newUser = new User(enteredEmail, enteredPassword);
	const existingUser = await newUser.getUserWithEmail();
  ...
  function executeLogout(req, res) {
